...
program initializes the PB6 and PB7 pins, sets up the interrupt on PB6, and runs an infinite loop 
...
void PortF_Init(void){
  ...
  GPIO_PORTF_PCTL_R &= ~0x0000FFF0; 	// configure PF3,2,1 as GPIO **VERIFY THIS**
...
void GPIOPortB_Handler(void){
    ...
    // or after a timeout. The maximum distance can be detected is 400cm.
		...
		SysPWM_RGB_LED(distance);	// dynamically change white brightness and or send distance to SysPWM module
		done = 1;
	}
